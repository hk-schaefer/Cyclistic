...
saveRDS(df_trips_all_raw, "~/R/CapStone/Output_data_all/df_trips_all_raw2.rds")
```
\

```{r read from HD, eval=FALSE, include=FALSE}
df_trips_all_raw <- readRDS("~/R/CapStone/Output_data_all/df_trips_all_raw2.rds")
```
\
...
We make the decision to set the lower threshold to 1 minute. Rides less than 1 minute can be classified as bike management to dock and release bikes in order to reset the rent status. For more details see DIVVY pricing program in chapter [IV. Appendix](#iv.-appendix).
\
\
...
```{r - read Top 20 station images, echo=FALSE, fig.align='center', fig.show='hold', out.width="66%"}
...
```{r read density distribution images, echo=FALSE, fig.align='center', fig.show='hold', out.width="66%"}
knitr::include_graphics(
